...
// window.find() sometimes gets stuck in an infinite loop (e.g., Yahoo search result pages).
// Abort after failing too many times.
...
function highlightAllMatches(text) {
  ...
  // recording the anchor and rects should give us enough of a fingerprint to detect
  // that we've already hit this match.
  var matches = [];
...
function containsRect(matches, rect) {
  for (var matchRect of matches) {
    if (matchRect.top === rect.top &&
        matchRect.left === rect.left &&
        matchRect.width === rect.width &&
        matchRect.height === rect.height) {
      return true;
    }
  }
